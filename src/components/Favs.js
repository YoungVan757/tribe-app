import React, { Component } from 'react';
import firebase from 'firebase'

export default class Favs extends Component {
    constructor(props) {
        super(props);
        this.state={
            following:false,
            tribe: []
        };
    }

    componentDidMount() {
        this.fetchData();
      }

      fetchData() {
        firebase
          .database()
          .ref()
          .once('value')
          .then(() => {this.fetchData()});
          };

}
          this.setState({
            following: snapshot.val().following
          });
        
    toggleLike() {
                  const currentTribe = {
                    id: 'car-tribe'
                  };
                  const currentUser = {
                    id: window.localStorage.getItem('tribe_uid')
                  };
                  const updates = {};
              
                    updates[`tribes/${currentTribe.id}/userId/
                                    ${currentUser.id}/following`] = true;
                 
                    firebase.database()
                    .ref()
                    .update(updates)
                    .then(() => {
                      this.fetchData();
                    });
          
                    document.getElementsByClassName("follow__button")[0]
                    .style.backgroundColor = "firebrick";
                  }   
    render() {

      const followClass = this.state.following ? 'following' : 'following-not';

        return<div className="favs">
            <button className='follow__button' onClick={() => this.toggleLike()} 
            className={followClass}
            ><img width="200" height="200" src="data:image/svg+xml;base64,
            
            MDYtMy41MjItMy4yNDgtNS43MTktMy4yNDh6Ii8+PC9zdmc+"></img>
            </button> 
            </div>
        
    } 
      